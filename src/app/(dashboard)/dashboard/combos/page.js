...
// Skeleton preserving the sortable list layout (same container + row size,
// dark-mode aware). Shown only while the dnd-kit chunk loads.
function ModelListSkeleton() {
  return (
    <div className="flex max-h-[55vh] min-w-0 flex-col gap-1 overflow-y-auto sm:max-h-[350px]" aria-hidden="true">
      {[0, 1, 2].map((i) => (
        <div key={i} className="flex min-w-0 items-center gap-1.5 rounded-md px-2 py-1 bg-black/[0.02] dark:bg-white/[0.02]">
          <div className="h-4 w-4 shrink-0 rounded bg-black/10 dark:bg-white/10 animate-pulse" />
          <div className="h-4 flex-1 rounded bg-black/10 dark:bg-white/10 animate-pulse" />
        </div>
      ))}
    </div>
  );
...
// Draggable list loads on demand so @dnd-kit stays out of the initial bundle.
// Non-drag parts of the page (header, cards, modals) render immediately.
const SortableModelList = dynamic(() => import("./components/SortableModelList"), {
  ssr: false,
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <SortableModelList
              ...
              onRemoveItem={handleRemoveModel}
            />
            )}
