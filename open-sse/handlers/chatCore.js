...
import { shouldBypass, createStreamCompatResponse } from "../streamCompat/index.js";
import { dedupeTools } from "../utils/toolDeduper.js";
import { takeRenamedToolNames } from "../utils/opencodeFingerprint.js";
import { injectCaveman } from "../rtk/caveman.js";
import { injectPonytail } from "../rtk/ponytail.js";
import { compressMessages, formatRtkLog, stripSystemBloat } from "../rtk/index.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking }) {
  ...
  // Bloat: strip system-prompt filler (fail-open inside; gated with RTK saver family)
  if (tokenSaverEnabled && rtkEnabled) {
    stripSystemBloat(translatedBody, finalFormat);
    xf.push("BLOAT");
  }
  ...
  try {
    ...
    if (stream && shouldBypass(provider, [error])) {
      try {
        log?.info?.("STREAM_COMPAT", `${provider.toUpperCase()} | ${model} | Stream failed (${error.message}), falling back to non-streaming`);
        const fallbackRes = await executor.execute({
          model,
          body: { ...translatedBody, stream: false },
          stream: false,
          credentials,
          providerSessionId: sessionSeed,
          clientTool,
        ...
        if (fallbackRes?.response?.ok) {
          const resText = await fallbackRes.response.text();
          streamController.handleComplete();
          return createStreamCompatResponse(resText, stripThinkingSuffix(upstreamModel));
        }
      } catch (fallbackErr) {
        log?.warn?.("STREAM_COMPAT", `${provider.toUpperCase()} | Fallback failed: ${fallbackErr.message}`);
      }
    }
    trackPendingRequest(model, provider, connectionId, false, true);
